...
func TestConfigIsUserAdmin(t *testing.T) {
	...
	require.False(t, config.IsAdmin(user), "invalid admin status")

	config.Admins = append(config.Admins, "admin")

	require.True(t, config.IsAdmin(user), "invalid admin status")
	require.True(t, config.IsAdmin(user), "invalid admin status")
}

func TestDisableAutoClean(t *testing.T) {
	config := NewConfiguration()
	require.True(t, config.AutoClean(), "invalid auto clean status")
	config.DisableAutoClean()
	require.False(t, config.AutoClean(), "invalid auto clean status")
}

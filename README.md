...
To migrate from one metadata backend to another it's possible to export and then to import metadata

```sh
$ ./plikd --config ./plikd.sqlite.cfg export /tmp/plik.metadata
$ ./plikd --config ./plikd.pg.cfg import /tmp/plik.metadata
```

To migrate from 1.2.4 user the --export flag

```sh
$ ./plikd --config ./plikd-1.2.4.cfg --export /tmp/plik.metadata
$ ./plikd --config ./plikd-1.3.cfg import /tmp/plik.metadata
```
